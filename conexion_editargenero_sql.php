...
try {
    //Variables...
$vcod_genero = filter_var($_POST['cod_genero']);
$vdescripcion = filter_var($_POST['descripcion']);


$update = $connec->prepare("UPDATE genero SET descripcion = :A
WHERE cod_genero = :claveprimaria");



$update->bindParam(':A', $vdescripcion);
$update->bindParam(':claveprimaria', $vcod_genero);


$update->execute();

header("location: genero_smec.php");
exit();
